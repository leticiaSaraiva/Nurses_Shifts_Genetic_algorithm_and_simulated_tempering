...
def gerar_estado_aleatorio():
    res = []
    for i in range(en*tu):
        res.append(randint(0,1))
    return res

def sucessor_aleatorio (e):
    succ = e[:] #cria uma copia do estado atual
    i_aleatorio = randint(0,tu*en-1) #seleciona uma posicao aleatoria para alterar
    succ[i_aleatorio] = (succ[i_aleatorio] + 1)%2 #altera o bit da posicao escolhida
    return succ
